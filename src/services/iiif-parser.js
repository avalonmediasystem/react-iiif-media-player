...
export default class IIIFParser {
                 ...
                 buildManifestMap(manifest) {
                   ...
                   if (obj.hasSequences === true) {
                     obj.hasCanvases = !!manifest.sequences[0]
                       .canvases;
                     if (obj.hasCanvases === true) {
                       obj.hasMultipleCanvases =
                 ...
                 createStructure(
                   ...
                   items.forEach((item, index) => {
                     if (
                       ...
                       if (
                         subMembers.length === 1 &&
                         subMembers[0].type === 'Canvas'
                       ) {
                         let structureLink = this.buildStructureLink(
                           item
                         );
                 ...
                 getCanvasByIndex(index, manifest) {
                   ...
                   const manifestMap = this.buildManifestMap(
                     manifest
                   );
                   let sequences = [];
                   let canvases = [];
                   ...
                   if (manifestMap.hasSequences) {
                     ...
                     sequences.forEach(sequence => {
                       ...
                       canvases.forEach(canvas => {
                         const canvasIndex = canvas.id.slice(
                           canvas.id.lastIndexOf('/') + 1
                         );
                 ...
                 getLabelValue(label) {
                   if (label && typeof label === 'object') {
                     ...
                     if (label.hasOwnProperty('@en')) {
                       return label['@en'].length > 0
                         ? label['@en'][0]
                         : '';
                     }
                     // None
                     if (label.hasOwnProperty('@none')) {
                       return label['@none'].length > 0
                         ? label['@none'][0]
                         : '';
                     }
                 ...
                 getPlayerDimensions(
                   ...
                   const canvasIndex = this.getCanvasIndex(
                     contentObj.id
                   );
                   const canvas = this.getCanvasByIndex(
                     canvasIndex,
                     manifest
                   );
                 ...
                 getChildCanvases(item) {
                   ...
                   const canvases = item.items.filter(
