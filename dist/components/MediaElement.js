  ...
  (0, _createClass2["default"])(MediaElement, [{
    ...
    value: function render() {
      ...
      var mediaBody = "".concat(sourceTags.join('\n'), "\n\t\t\t\t").concat(tracksTags.join('\n')),
          mediaHtml = props.mediaType === 'video' ? "<video id=\"".concat(props.id, "\" width=\"").concat(props.width, "\" height=\"").concat(props.height, "\"").concat(props.poster ? " poster=".concat(props.poster) : '', "\n\t\t\t\t\t").concat(props.controls ? ' controls' : '').concat(props.preload ? " preload=\"".concat(props.preload, "\"") : '', ">\n\t\t\t\t\t").concat(mediaBody, "\n\t\t\t\t</video>") : "<audio id=\"".concat(props.id, "\" width=\"").concat(props.width, "\" controls>\n\t\t\t\t\t").concat(mediaBody, "\n\t\t\t\t</audio>");
